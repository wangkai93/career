...
public abstract class ExcelHelper {
	...
	protected <T> Object parseValueWithType(String value, Class<?> type) {
		...
		try { // 根据属性的类型将内容转换成对应的类型
			if (Boolean.TYPE == type || type.getSimpleName().equals("Boolean")) {
				result = Boolean.parseBoolean(value);
			} else if (Byte.TYPE == type|| type.getSimpleName().equals("Byte")) {
				result = Byte.parseByte(value);
			} else if (Short.TYPE == type|| type.getSimpleName().equals("Short")) {
				result = Short.parseShort(value);
			} else if (Integer.TYPE == type|| type.getSimpleName().equals("Integer")) {
				result = Integer.parseInt(value);
			} else if (Long.TYPE == type|| type.getSimpleName().equals("Long")) {
				result = Long.parseLong(value);
			} else if (Float.TYPE == type|| type.getSimpleName().equals("Float")) {
				result = Float.parseFloat(value);
			} else if (Double.TYPE == type|| type.getSimpleName().equals("Double")) {
